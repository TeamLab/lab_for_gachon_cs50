...
함수를 작성하고 나서 잘 작동하는지 궁금할 것이다. 확인하는 방법은 두 가지가 있다. 먼저 아래처럼 기존 코드에서 `main` 함수를 주석 처리하고 테스트 코드를 삽입하는 방법이다. `fahrenheit_converter.py` 파일을 `vim editor`로 열어 맨 마지막 `if __name__ == '__main__':`을 아래처럼 수정해 보자
...
화씨온도 :  20.3
```
어떻게 보면 굉장히 간단한 함수이다. 입력받은 값을 그대로 출력만 해주면 되기 때문이다.  단지 `섭씨온도`와 `화씨온도`등 메세지가 정확히 출력되도록 오타없이 입력해주기 바란다.
...
굉장히 간단하지만 용어가 익숙치 않아 헷갈릴수 있다. 함수는 우리가 만들어 쓰는 것도 있지만 이미 파이썬에서 제공해주는 함수를 사용하는 경우도 많다. 예를 들면 `print` 나 `input` 같은 것들이 그런 종류의 함수이다. 이런 함수들을 built-in 함수 라고 한다. 아래 코드를 보자.
```python
...
## 결과 출력하기 
실제 코드가 다 작성되어 `python3.4 fahrenheit_converter.py`실행하면 아래와 같이 결과를 볼 수 있을 것이다. 당연히 입력 부분은 수강자가 직접 입력을 해주어야 프로그램 진행된다.
